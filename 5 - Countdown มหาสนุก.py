# ---------------------------------------------------------
#   *** Fun with countdown ***
#
#  Name: sorawit loeyvanicharoen
#  ID: 64010876
#  Exercise: Chapter : 1 - item : 5 - Countdown มหาสนุก
# ---------------------------------------------------------

print(" *** Fun with countdown ***")

ls = [int(e) for e in input("Enter list : ").split()]
#print(ls)

total_countdown = 0
new_list =[]
sum_list =[]
this_list_is_OK = False
last_countdown_status = False

for i in range (0, len(ls)):
    the_first = int(ls[i])
    #print('i is >> ', i)
    if the_first == 1 and i < len(ls) -1 and this_list_is_OK != True and last_countdown_status == False:
        this_list_is_OK = False
        new_list.append(ls[i])
        sum_list.append(new_list)
        #print('i >> ', i, ' : len(ls) :', len(ls), ' :the first is 1 >>> sum_list', sum_list)      # for debugging
        new_list = []
        total_countdown += 1
    elif i+1 < len(ls):
         if this_list_is_OK != True:
             new_list.append(ls[i])
         the_second = int(ls[i+1])
         if (the_first - the_second) == 1:
             this_list_is_OK = True
             last_countdown_status = True
             new_list.append(ls[i+1])
             #print('inside the_first - the_second is 1, and last_count_status ' , last_countdown_status)
         else:
             new_list = []
         if int(ls[i+1] == 1):
             this_list_is_OK = False
             sum_list.append(new_list)
             #print(sum_list)                 # for debugging
             new_list = []
             total_countdown += 1
             exit
    #print(last_countdown_status, 'new list is : ', new_list,  ' :::: sum_list >> ', sum_list)     # for debuggin

temp_list = []
temp_list.append(sum_list)
#print('temp_list  ---> ', temp_list)
#print('sum_list  ---> ', sum_list)
sum_list = temp_list
sum_list.insert(0, total_countdown)
print(sum_list)





